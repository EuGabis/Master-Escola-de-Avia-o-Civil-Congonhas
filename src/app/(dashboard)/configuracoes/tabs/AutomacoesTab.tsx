    ...
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800">
      <header className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-800">
        ...
        <Button onClick={() => setEditing({ mode: "create" })}>
          <span className="flex items-center gap-2"><Plus size={14} /> Nova</span>
        </Button>
      </header>
      ...
      {loading ? (
        ...
        <div className="p-10 text-center">
          ...
          <p className="text-sm font-medium text-slate-900 dark:text-white">Sem automacoes ainda</p>
          <p className="text-xs text-slate-500 mt-1 max-w-md mx-auto">
            Crie regras para etiquetar, mover no Kanban ou atribuir conversas automaticamente.{" "}
            <strong>Ex:</strong> palavra &quot;preço&quot; aplica etiqueta <em>Financeiro</em>.
          </p>
        </div>
      ) : (
          ...
          {items.map((a) => {
            ...
            return (
              <div key={a.id} className={cn(
                "px-5 py-4 transition group",
                a.enabled ? "" : "opacity-60"
              )}>
                <div className="flex items-start gap-3">
                  <div className={cn(
                    "w-9 h-9 rounded-lg flex items-center justify-center shrink-0",
                    a.enabled ? "bg-master-orange/10 text-master-orange" : "bg-slate-100 dark:bg-slate-800 text-slate-400"
                  )}>
                    <Zap size={16} />
                  </div>
                  ...
                  <div className="flex items-center gap-1">
                    <button onClick={() => void toggle(a)} className={cn(
                      ...
                      a.enabled ? "text-emerald-500 hover:bg-emerald-500/10" : "text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800"
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        <div>
          <Label>Nome</Label>
          <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="ex: Classificar leads de Piloto" autoFocus />
        </div>
        <div>
          <Label>Gatilho (quando)</Label>
          <div className="grid grid-cols-2 gap-2">
            <button type="button" onClick={() => setTriggerType("keyword")}
              className={cn("rounded-lg border p-3 text-left text-sm transition",
                triggerType === "keyword" ? "border-master-orange bg-master-orange/5" : "border-slate-200 dark:border-slate-700 hover:border-master-orange/50")}>
            ...
            <button type="button" onClick={() => setTriggerType("first_message")}
              className={cn("rounded-lg border p-3 text-left text-sm transition",
                triggerType === "first_message" ? "border-master-orange bg-master-orange/5" : "border-slate-200 dark:border-slate-700 hover:border-master-orange/50")}>
        ...
        {triggerType === "keyword" && (
          <div>
            ...
            <Input value={keywords} onChange={(e) => setKeywords(e.target.value)} placeholder="ex: preço, valor, mensalidade" />
          </div>
        )}
        ...
        <div>
          ...
          <Input value={addLabelName} onChange={(e) => setAddLabelName(e.target.value)} placeholder="ex: Financeiro, PP, Urgente..." />
        </div>
        <div>
